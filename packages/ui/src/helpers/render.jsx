...
// Get sidebar Lucide icon component
export function getLucideIcon(key, selected = false, strokeWidth = 2) {
  const size = 16;
  const SELECTED_COLOR = 'var(--primary)';
  const UNSELECTED_COLOR = 'var(--text-tertiary)'; // Lighter gray for sidebar icons
  const iconColor = selected ? SELECTED_COLOR : UNSELECTED_COLOR;
  const commonProps = {
    size,
    strokeWidth,
  ...
  // Return different icons based on key
  switch (key) {
    case 'detail':
...
/**
 * Get vendor icon based on channel type
 * @param {number} channelType - Channel type value
 * @returns {JSX.Element|null} - Corresponding vendor icon component
 */
export function getChannelIcon(channelType) {
  ...
  switch (channelType) {
    ...
    case 15: // Baidu Wenxin Qianfan
    case 46: // Baidu Wenxin Qianfan V2
      return <Wenxin.Color size={iconSize} />;
    case 17: // Alibaba Tongyi Qianwen
      return <Qwen.Color size={iconSize} />;
    case 18: // iFlytek Xinghuo
      return <Spark.Color size={iconSize} />;
    case 16: // Zhipu ChatGLM
    case 26: // Zhipu GLM-4V
    ...
    case 8: // Custom channel
    case 22: // Knowledge base: FastGPT
      return <FastGPT.Color size={iconSize} />;
    case 21: // Knowledge base: AI Proxy
    case 44: // Embedding model: MokaAI M3E
    default:
      return null; // Unknown type or custom channel shows no icon
  }
}

/**
 * Dynamically get LobeHub icon component by icon name
 * Supports:
 * - Basic: "OpenAI", "OpenAI.Color", etc.
 * - Extra properties (dot notation): "OpenAI.Avatar.type={'platform'}", "OpenRouter.Avatar.shape={'square'}"
 * - Still compatible with second parameter size; if size= in string, string takes precedence
 * @param {string} iconName - Icon name/description
 * @param {number} size - Icon size, defaults to 14
 * @returns {JSX.Element} - Corresponding icon component or Avatar
 */
export function getLobeHubIcon(iconName, size = 14) {
  if (typeof iconName === 'string') iconName = iconName.trim();
  // If no icon name, return Avatar
  if (!iconName) {
    return (
      <Avatar size='sm'>
  ...
  // Parse component path and dot-notation properties
  ...
  // Parse dot-notation properties, like: key={...}, key='...', key="...", key=123, key, key=true/false
  const props = {};

  const parseValue = (raw) => {
    if (raw == null) return true;
    let v = String(raw).trim();
    // Remove one layer of curly braces
  ...
  // Compatible with second parameter size, if not explicitly specified in string, use function parameter
...
// Extended 20-color palette (10 < N ≤ 20)
...
export const modelColorMap = {
  ...
  // 'dall-e-2': 'rgb(147,112,219)', // between purple and blue
  'dall-e-3': 'rgb(153,50,204)', // between violet and magenta
  'gpt-3.5-turbo': 'rgb(184,227,167)', // light green
  // 'gpt-3.5-turbo-0301': 'rgb(131,220,131)', // bright green
  'gpt-3.5-turbo-0613': 'rgb(60,179,113)', // medium sea green
  ...
  'gpt-3.5-turbo-instruct': 'rgb(175,238,238)', // pale turquoise
  ...
  'text-davinci-003': 'rgb(219,112,147)', // pale violet red (same as Curie, same series)
  ...
  'text-moderation-stable': 'rgb(255,160,122)', // light coral (same as Babbage, same function type)
...
export function modelToColor(modelName) {
  // 1. If model is in predefined modelColorMap, use predefined color
  if (modelColorMap[modelName]) {
    return modelColorMap[modelName];
  }

  // 2. Generate a stable number as index
  let hash = 0;
  ...
  // 3. Select different color palette based on model name length
  const colorPalette = modelName.length > 10 ? extendedColors : baseColors;

  // 4. Use hash value to select color
...
// Render model tag with model icon
export function renderModelTag(modelName, options = {}) {
  ...
  return (
    <Badge
      variant='secondary'
      className={`rounded-full px-2 py-0.5 text-xs font-medium cursor-pointer ${getColorClass(color || stringToColor(modelName))}`}
      onClick={onClick}
    >
...
export function renderGroup(group) {
  if (group === '') {
    return (
      <Badge
        variant='secondary'
        className={`rounded-full px-2 py-0.5 text-xs font-medium ${getColorClass('white')}`}
      >
        {i18next.t('Your default group')}
      </Badge>
    );
  }
    ...
    <span key={group} className='flex flex-wrap gap-1'>
      {groups.map((groupItem) => (
        <Badge
          variant='secondary'
          key={groupItem}
          className={`rounded-full px-2 py-0.5 text-xs font-medium cursor-pointer ${getColorClass(tagColors[groupItem] || groupToColor(groupItem))}`}
          onClick={async (event) => {
            event.stopPropagation();
            if (await copy(groupItem)) {
              showSuccess(i18next.t('Copied:') + groupItem);
            } else {
              showError(
                i18next.t('Unable to copy to clipboard, please copy manually') +
                  ': ' +
                  groupItem,
              );
            }
...
export function renderRatio(ratio) {
  ...
  return (
    <Badge
      variant='secondary'
      className={`rounded-full px-2 py-0.5 text-xs font-medium ${getColorClass(color)}`}
    >
      {ratio}x {i18next.t('Ratio')}
    </Badge>
  );
}
...
export const renderGroupOption = (item) => {
  ...
  const baseStyle = {
    ...
    backgroundColor: focused ? 'var(--fill-0)' : 'transparent',
    opacity: disabled ? 0.5 : 1,
    ...(selected && {
      backgroundColor: 'var(--primary)',
    }),
    '&:hover': {
      backgroundColor: !disabled && 'var(--fill-1)',
    },
  };
...
/**
 * Convert USD amount to currently selected currency
...
function getEffectiveRatio(groupRatio, user_group_ratio) {
  ...
  const ratioLabel = useUserGroupRatio
    ? i18next.t('Exclusive group ratio')
    : i18next.t('Group ratio');
...
function renderPriceSimpleCore({
  ...
  if (modelPrice !== -1) {
    ...
    return i18next.t('Price: {{symbol}}{{price}} * {{ratioType}}: {{ratio}}', {
      symbol: symbol,
      price: displayPrice,
  ...
  const parts = [];
  // base: model ratio
  parts.push(i18next.t('Model: {{ratio}}'));

  // cache part (label differs when with image)
  if (shouldShowCache) {
    parts.push(i18next.t('Cache: {{cacheRatio}}'));
  }
  ...
  if (hasSplitCacheCreation) {
    if (shouldShowCacheCreation5m && shouldShowCacheCreation1h) {
      parts.push(
        i18next.t(
          'Cache creation: 5m {{cacheCreationRatio5m}} / 1h {{cacheCreationRatio1h}}',
        ),
      );
    } else if (shouldShowCacheCreation5m) {
      parts.push(i18next.t('Cache creation: 5m {{cacheCreationRatio5m}}'));
    } else if (shouldShowCacheCreation1h) {
      parts.push(i18next.t('Cache creation: 1h {{cacheCreationRatio1h}}'));
    }
  } else if (shouldShowLegacyCacheCreation) {
    parts.push(i18next.t('Cache creation: {{cacheCreationRatio}}'));
  }

  // image part
  if (image) {
    parts.push(i18next.t('Image input: {{imageRatio}}'));
  }
  ...
  if (isSystemPromptOverride) {
    result += '\n\r' + i18next.t('System prompt override');
  }
...
export function renderModelPrice(
  ...
  if (modelPrice !== -1) {
    ...
    return i18next.t(
      'Model price: {{symbol}}{{price}} * {{ratioType}}: {{ratio}} = {{symbol}}{{total}}',
      {
        symbol: symbol,
        price: displayPrice,
        ratio: groupRatio,
    ...
    return (
      <>
        <article>
          <p>
            {i18next.t(
              'Input price: {{symbol}}{{price}} / 1M tokens{{audioPrice}}',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
                audioPrice: audioInputSeperatePrice
                  ? `, audio ${symbol}${(audioInputPrice * rate).toFixed(6)} / 1M tokens`
                  : '',
              },
            )}
          </p>
          <p>
            {i18next.t(
              'Output price: {{symbol}}{{price}} * {{completionRatio}} = {{symbol}}{{total}} / 1M tokens (Completion ratio: {{completionRatio}})',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
          ...
          {cacheTokens > 0 && (
            <p>
              {i18next.t(
                'Cache price: {{symbol}}{{price}} * {{cacheRatio}} = {{symbol}}{{total}} / 1M tokens (Cache ratio: {{cacheRatio}})',
                {
                  symbol: symbol,
                  price: (inputRatioPrice * rate).toFixed(6),
          ...
          {image && imageOutputTokens > 0 && (
            <p>
              {i18next.t(
                'Image input price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens (Image ratio: {{imageRatio}})',
                {
                  symbol: symbol,
          ...
          {webSearch && webSearchCallCount > 0 && (
            <p>
              {i18next.t(
                'Web Search Price: {{symbol}}{{price}} / 1K requests',
                {
                  symbol: symbol,
          ...
          {fileSearch && fileSearchCallCount > 0 && (
            <p>
              {i18next.t('File search price: {{symbol}}{{price}} / 1K times', {
                symbol: symbol,
          ...
          {imageGenerationCall && imageGenerationCallPrice > 0 && (
            <p>
              {i18next.t(
                'Image generation call: {{symbol}}{{price}} / 1 time',
                {
                  symbol: symbol,
          ...
          <p>
            {(() => {
              ...
              if (image && imageOutputTokens > 0) {
                inputDesc = i18next.t(
                  '(Input {{nonImageInput}} tokens + Image input {{imageInput}} tokens * {{imageRatio}} / 1M tokens * {{symbol}}{{price}}',
                  {
                ...
                inputDesc = i18next.t(
                  '(Input {{nonCacheInput}} tokens / 1M tokens * {{symbol}}{{price}} + Cache {{cacheInput}} tokens / 1M tokens * {{symbol}}{{cachePrice}}',
                  {
                ...
                inputDesc = i18next.t(
                  '(Input {{nonAudioInput}} tokens / 1M tokens * {{symbol}}{{price}} + Audio input {{audioInput}} tokens / 1M tokens * {{symbol}}{{audioPrice}}',
                  {
                ...
                inputDesc = i18next.t(
                  '(Input {{input}} tokens / 1M tokens * {{symbol}}{{price}}',
                  {
                    input: inputTokens,
                    symbol: symbol,
                    price: (inputRatioPrice * rate).toFixed(6),
              ...
              const outputDesc = i18next.t(
                'Output {{completion}} tokens / 1M tokens * {{symbol}}{{compPrice}}) * {{ratioType}} {{ratio}}',
                {
              ...
              // Build extra services description
              const extraServices = [
                webSearch && webSearchCallCount > 0
                  ? i18next.t(
                      ' + Web search {{count}} times / 1K times * {{symbol}}{{price}} * {{ratioType}} {{ratio}}',
                      {
                ...
                fileSearch && fileSearchCallCount > 0
                  ? i18next.t(
                      ' + File search {{count}} times / 1K times * {{symbol}}{{price}} * {{ratioType}} {{ratio}}',
                      {
                ...
                imageGenerationCall && imageGenerationCallPrice > 0
                  ? i18next.t(
                      ' + Image generation call {{symbol}}{{price}} / 1 time * {{ratioType}} {{ratio}}',
                      {
                        symbol: symbol,
                        price: (imageGenerationCallPrice * rate).toFixed(6),
                        ratio: groupRatio,
          ...
          <p>
            {i18next.t('For reference only, actual deduction shall prevail')}
          </p>
        </article>
      </>
    );
...
export function renderLogContent(
  ...
  if (modelPrice !== -1) {
    return i18next.t(
      'Model price {{symbol}}{{price}}, {{ratioType}} {{ratio}}',
      {
        symbol: symbol,
    ...
    if (image) {
      return i18next.t(
        'Model ratio {{modelRatio}}, cache ratio {{cacheRatio}}, completion ratio {{completionRatio}}, image input ratio {{imageRatio}}, {{ratioType}} {{ratio}}',
        {
      ...
      return i18next.t(
        'Model ratio {{modelRatio}}, cache ratio {{cacheRatio}}, completion ratio {{completionRatio}}, {{ratioType}} {{ratio}}, Web search called {{webSearchCallCount}} times',
        {
          modelRatio: modelRatio,
          cacheRatio: cacheRatio,
          completionRatio: completionRatio,
      ...
      return i18next.t(
        'Model ratio {{modelRatio}}, cache ratio {{cacheRatio}}, completion ratio {{completionRatio}}, {{ratioType}} {{ratio}}',
        {
          modelRatio: modelRatio,
          cacheRatio: cacheRatio,
          completionRatio: completionRatio,
...
export function renderAudioModelPrice(
  ...
  if (modelPrice !== -1) {
    ...
    // *2 because 1 ratio = 0.002 USD, do not remove
    let inputRatioPrice = modelRatio * 2.0;
    let completionRatioPrice = modelRatio * 2.0 * completionRatio;
    let cacheRatioPrice = modelRatio * 2.0 * cacheRatio;
    ...
    return (
      <>
        <article>
          <p>
            {i18next.t('Prompt price: {{symbol}}{{price}} / 1M tokens', {
              symbol: symbol,
              price: (inputRatioPrice * rate).toFixed(6),
            })}
          </p>
          <p>
            {i18next.t(
              'Completion price: {{symbol}}{{price}} * {{completionRatio}} = {{symbol}}{{total}} / 1M tokens (Completion ratio: {{completionRatio}})',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
                total: (completionRatioPrice * rate).toFixed(6),
          ...
          <p>
            {i18next.t(
              'Audio prompt price: {{symbol}}{{price}} * {{audioRatio}} = {{symbol}}{{total}} / 1M tokens (Audio ratio: {{audioRatio}})',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
          ...
          <p>
            {i18next.t(
              'Audio completion price: {{symbol}}{{price}} * {{audioRatio}} * {{audioCompRatio}} = {{symbol}}{{total}} / 1M tokens (Audio completion ratio: {{audioCompRatio}})',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
                total: (
          ...
          <p>
            {cacheTokens > 0
              ? i18next.t(
                  'Text prompt {{nonCacheInput}} tokens / 1M tokens * {{symbol}}{{price}} + Cache {{cacheInput}} tokens / 1M tokens * {{symbol}}{{cachePrice}} + Text completion {{completion}} tokens / 1M tokens * {{symbol}}{{compPrice}} = {{symbol}}{{total}}',
                  {
                    nonCacheInput: inputTokens - cacheTokens,
                    cacheInput: cacheTokens,
                    symbol: symbol,
              ...
              : i18next.t(
                  'Text prompt {{input}} tokens / 1M tokens * {{symbol}}{{price}} + Text completion {{completion}} tokens / 1M tokens * {{symbol}}{{compPrice}} = {{symbol}}{{total}}',
                  {
                    input: inputTokens,
                    symbol: symbol,
                    price: (inputRatioPrice * rate).toFixed(6),
          ...
          <p>
            {i18next.t(
              'Audio prompt {{input}} tokens / 1M tokens * {{symbol}}{{audioInputPrice}} + Audio completion {{completion}} tokens / 1M tokens * {{symbol}}{{audioCompPrice}} = {{symbol}}{{total}}',
              {
          ...
          <p>
            {i18next.t(
              'Total price: text price {{textPrice}} + audio price {{audioPrice}} = {{symbol}}{{total}}',
              {
                symbol: symbol,
                total: (price * rate).toFixed(6),
...
export function renderClaudeModelPrice(
  ...
  if (modelPrice !== -1) {
    ...
    const breakdownSegments = [
      i18next.t('Prompt {{input}} tokens / 1M tokens * {{symbol}}{{price}}', {
        input: inputTokens,
        symbol,
    ...
    if (shouldShowCache) {
      breakdownSegments.push(
        i18next.t(
          'Cache {{tokens}} tokens / 1M tokens * {{symbol}}{{price}} (ratio: {{ratio}})',
          {
            tokens: cacheTokens,
            symbol,
    ...
    if (shouldShowLegacyCacheCreation) {
      breakdownSegments.push(
        i18next.t(
          'Cache creation {{tokens}} tokens / 1M tokens * {{symbol}}{{price}} (ratio: {{ratio}})',
          {
    ...
    if (shouldShowCacheCreation5m) {
      breakdownSegments.push(
        i18next.t(
          '5m cache creation {{tokens}} tokens / 1M tokens * {{symbol}}{{price}} (ratio: {{ratio}})',
          {
    ...
    if (shouldShowCacheCreation1h) {
      breakdownSegments.push(
        i18next.t(
          '1h cache creation {{tokens}} tokens / 1M tokens * {{symbol}}{{price}} (ratio: {{ratio}})',
          {
    ...
    breakdownSegments.push(
      i18next.t(
        'Completion {{completion}} tokens / 1M tokens * {{symbol}}{{price}}',
        {
          completion: completionTokens,
          symbol,
    ...
    return (
      <>
        <article>
          ...
          <p>
            {i18next.t(
              'Completion price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens',
              {
                symbol: symbol,
                price: (inputRatioPrice * rate).toFixed(6),
          ...
          {shouldShowCache && (
            <p>
              {i18next.t(
                'Cache price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens (Cache ratio: {{cacheRatio}})',
                {
                  symbol: symbol,
                  price: (inputRatioPrice * rate).toFixed(6),
                  ratio: cacheRatio,
          ...
          {shouldShowLegacyCacheCreation && (
            <p>
              {i18next.t(
                'Cache creation price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens (Cache creation ratio: {{cacheCreationRatio}})',
                {
                  symbol: symbol,
                  price: (inputRatioPrice * rate).toFixed(6),
                  ratio: cacheCreationRatio,
          ...
          {shouldShowCacheCreation5m && (
            <p>
              {i18next.t(
                '5m cache creation price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens (5m cache creation ratio: {{cacheCreationRatio5m}})',
                {
                  symbol: symbol,
                  price: (inputRatioPrice * rate).toFixed(6),
                  ratio: cacheCreationRatio5m,
          ...
          {shouldShowCacheCreation1h && (
            <p>
              {i18next.t(
                '1h cache creation price: {{symbol}}{{price}} * {{ratio}} = {{symbol}}{{total}} / 1M tokens (1h cache creation ratio: {{cacheCreationRatio1h}})',
                {
                  symbol: symbol,
                  price: (inputRatioPrice * rate).toFixed(6),
                  ratio: cacheCreationRatio1h,
          ...
          {shouldShowCacheCreation5m && shouldShowCacheCreation1h && (
            <p>
              {i18next.t(
                'Cache creation price total: 5m {{symbol}}{{five}} + 1h {{symbol}}{{one}} = {{symbol}}{{total}} / 1M tokens',
                {
                  symbol: symbol,
...
export function renderClaudeLogContent(
  ...
  if (modelPrice !== -1) {
    ...
    if (hasSplitCacheCreation) {
      if (shouldShowCacheCreation5m && shouldShowCacheCreation1h) {
        cacheCreationPart = i18next.t(
          'Cache creation ratio 5m {{cacheCreationRatio5m}} / 1h {{cacheCreationRatio1h}}',
          {
            cacheCreationRatio5m,
            cacheCreationRatio1h,
          },
        ...
        cacheCreationPart = i18next.t(
          'Cache creation ratio 5m {{cacheCreationRatio5m}}',
          {
            cacheCreationRatio5m,
          },
        );
        ...
        cacheCreationPart = i18next.t(
          'Cache creation ratio 1h {{cacheCreationRatio1h}}',
          {
            cacheCreationRatio1h,
          },
        );
    ...
    if (!cacheCreationPart) {
      cacheCreationPart = i18next.t(
        'Cache creation ratio {{cacheCreationRatio}}',
        {
          cacheCreationRatio,
        },
      );
    ...
    const parts = [
      i18next.t('Model ratio {{modelRatio}}', { modelRatio }),
      i18next.t('Output ratio {{completionRatio}}', { completionRatio }),
      i18next.t('Cache ratio {{cacheRatio}}', { cacheRatio }),
      cacheCreationPart,
...
// Unified into renderModelPriceSimple, if there are remaining references, change to pass provider='claude'

/**
 * rehype plugin: Split paragraph and other text nodes into word-by-word <span>, and add fade-in animation class.
 * Only used during streaming rendering phase, to avoid re-animating already rendered text.
 */
export function rehypeSplitWordsIntoSpans(options = {}) {
  const { previousContentLength = 0 } = options;

  return (tree) => {
    let currentCharCount = 0; // Number of characters processed so far
    ...
    visit(tree, 'element', (node) => {
      if (
        ...
        node.children.forEach((child) => {
          if (child.type === 'text') {
            try {
              // Use Intl.Segmenter to precisely split Chinese and English text with punctuation
              ...
              Array.from(segments)
                ...
                .forEach((word) => {
                  ...
                  // Determine if this word is new (after previousContentLength)
