-- BillClaw D1 Schema Migration v1
-- Auto-created on first deploy via Deploy Button
--
-- This migration initializes the database schema for BillClaw on Cloudflare D1.
-- It creates tables for accounts, transactions, and sync state.
--
-- Usage:
--   wrangler d1 execute <database> --file=./src/migrations/0001_init.sql
--
-- Or use migrations_dir in wrangler.toml for automatic migrations

-- Accounts table
-- Stores registered account information for both Plaid and Gmail connections
...
-- Transactions table
-- Stores all transaction records with efficient indexing for date-based queries
...
-- Sync state table
-- Stores sync cursors and state for incremental transaction updates
...
-- These optimize the most frequent access patterns
